...
top left to the bottom right, by moving left, right, up, and
down, is indicated in bold red and is equal to 2297.
...
class Matrix(object):
    ...
    def next_nodes(self, start):
        next_node1, next_node2, next_node3, next_node4 = None, None, None, None
        # go down
        ...
        # go left
        if start[1] - 1 >= 0:
            next_node4 = (start[0], start[1] - 1)

        return next_node1, next_node2, next_node3, next_node4
...
class Heap(object):
    ...
    def heapify(self, graph, start):
        next_node1, next_node2, next_node3, next_node4 = graph.next_nodes(start)
        ...
        if next_node4 is not None and not graph.goto_index(next_node4).explored:
            greedy_score = graph.goto_index(start).greedy_score + graph.goto_index(next_node4).length
            self.add_heap_node(greedy_score, next_node4)
